...
def knapsack(P, M, N, items, constraints):
  ...
  for i in range(1,N,1):
      ...
      cost = int(cost*100.0)
      resale_value = int(resale_value*100.0)
      print("here2")
...
def solve(P, M, N, C, items, constraints):
  ...
  items = basic_preprocess(P, M, N, items) # Making sure no items are not profitable/don't weigh more than necessary
  ...
  # output = {items[0]}
  # for item in items:
  #   print(constraint_checker(output, item, constraints))
  # return ["Dog","Cat"]
